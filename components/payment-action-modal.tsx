  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      updateMaintenancePayment(payment.id, {
        ...
        updatedBy: "admin", // En una app real, usar el ID del admin actual
      })

      // Marcar multas como pagadas si las hay en el breakdown
      if (payment.breakdown?.fines) {
        payment.breakdown.fines.forEach((fine) => {
              ...
              <div className="space-y-2">
                ...
                {payment.breakdown.fines && payment.breakdown.fines.length > 0 && (
                  <div>
                    <div className="flex justify-between font-medium">
                      <span>Multas:</span>
                      <span>
                        ${payment.breakdown.fines.reduce((sum, fine) => sum + fine.amount, 0).toLocaleString()}
                      </span>
                    </div>
                    {payment.breakdown.fines.map((fine) => (
                    ...
                    <div className="flex justify-between font-medium">
                      ...
                      <span>
                        ...
                        {payment.breakdown.agreements
                          .reduce((sum, agreement) => sum + agreement.amount, 0)
                          .toLocaleString()}
                      </span>
                    </div>
                    {payment.breakdown.agreements.map((agreement) => (
                ...
                {payment.breakdown.advancePayments && payment.breakdown.advancePayments.length > 0 && (
                  <div>
                    ...
                    {payment.breakdown.advancePayments.map((advance, index) => (
