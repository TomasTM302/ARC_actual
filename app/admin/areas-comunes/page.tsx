...
export default function AdminCommonAreasPage() {
  ...
  const getDaysInCalendar = () => {
    ...
    while (days.length % 7 !== 0) {
      const nextDate = new Date(year, month, lastDay.getDate() + (days.length - (startDay + lastDay.getDate())) + 1);
      days.push({
        date: nextDate,
        isCurrentMonth: false,
      });
  ...
  const handleEditArea = (area) => {
    ...
    setSelectedArea({
      ...
      activo: area.activo ?? (area.isActive ? 'Activo' : 'Inactivo'),
  ...
  return (
    <AuthGuard requireAuth requireAdmin>
                  ...
                  <table className="w-full">
                    ...
                    <tbody>
                      {reservaciones.length === 0 ? (
                        ...
                        reservaciones.map((reservation: any) => {
      ...
      <span className={`px-3 py-1 rounded-full text-xs font-semibold ${area.activo === 'Activo' ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-500'}`}>{area.activo === 'Activo' ? 'Habilitada' : 'Deshabilitada'}</span>
                        ...
                        <div className="mb-1"><strong>Anticipación máxima:</strong> 7 días</div>
                        <div className="mb-1"><strong>Costo de la Reserva</strong> {area.maxDuration} </div>
...
<Button
  ...
  onClick={async () => {
    ...
    if (!window.confirm(`¿Seguro que deseas ${accion} el área "${area.name}"?`)) return;
    try {
