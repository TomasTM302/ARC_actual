...
function calculateIncomeByCategory(payments = [], reservations = [], months) {
        ...
        if (monthIndex >= 0) {
          ...
          if (payment.breakdown) {
            ...
            if (payment.breakdown.maintenance) {
              incomeData.maintenance[monthIndex] += payment.breakdown.maintenance
            }

            // Cuotas recuperadas
            if (payment.breakdown.recoveredPayments) {
              const recoveredTotal = payment.breakdown.recoveredPayments.reduce((sum, item) => sum + item.amount, 0)
              incomeData.recovered[monthIndex] += recoveredTotal
            }

            // Cuotas adelantadas
            if (payment.breakdown.advancePayments) {
              const advanceTotal = payment.breakdown.advancePayments.reduce((sum, item) => sum + item.amount, 0)
              incomeData.advance[monthIndex] += advanceTotal
            }

            // Multas
            if (payment.breakdown.fines) {
              const finesTotal = payment.breakdown.fines.reduce((sum, fine) => sum + fine.amount, 0)
              incomeData.fines[monthIndex] += finesTotal
            }

            // Convenios
            if (payment.breakdown.agreements) {
              const agreementsTotal = payment.breakdown.agreements.reduce((sum, agreement) => sum + agreement.amount, 0)
              incomeData.agreements[monthIndex] += agreementsTotal
            }

            // Otros ingresos
            if (payment.breakdown.others) {
              const othersTotal = payment.breakdown.others.reduce((sum, item) => sum + item.amount, 0)
...
export default function EstadoResultadosPage() {
  ...
  // Obtener los pagos reales del store
  const { maintenancePayments } = useAppStore()
  // Obtener las reservaciones de áreas comunes (si existe en el store)
  const { commonAreaReservations = [] } = useAppStore()
